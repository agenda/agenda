...
var Agenda = module.exports = function(config, cb) {
  ...
  // Load adapter and initialize it. Default to mongodb
  if (!config.adapter || config.adapter == 'mongodb') {
    DBAdapter = require('./db_adapters/mongodb');
  } else {
    if (typeof config.adapter === 'string') {
      DBAdapter = require("agenda-db-" + config.adapter);
    } else {
      DBAdapter = config.adapter;
    }
  }

  this._dbAdapter = new DBAdapter(this, config, cb);
};
...
// Deprecated in favor of useing a more generic api
Agenda.prototype.mongo = function( mdb, collection, cb ){
  console.warn("Agenda.mongo deprecated in favor Agenda.connection");
  this._dbAdapter.connection(mdb, collection, cb);

  return this;
};

Agenda.prototype.connection = function( db, collection, cb ){
  this._dbAdapter.connection(db, collection, cb);
 ...
 *      authenticate against the Admin DB and then pass the DB instance in to the Constructor or use Agenda.connection().
 *    - If your app already has a DB connection then use that. ie. specify config.connection in the Constructor or use Agenda.connection().
 */
Agenda.prototype.database = function(url, collection, options, cb) {
  this._dbAdapter.database(url, collection, options, cb);
...
Agenda.prototype.db_init = function( collection, cb ){
  this._dbAdapter.db_init(collection, cb);
};
...
Agenda.prototype.jobs = function( query, cb ){
  ...
  this._dbAdapter.jobs(query, function(err, result) {
    var jobs;
    if( !err ){
...
Agenda.prototype.purge = function(cb) {
  ...
  this._dbAdapter.purge(definedNames, cb);
};
...
/** Cancels any jobs matching the passed native query, and removes them from the database.
 *  @param query native db query
 *  @param cb callback( error, numRemoved )
 *
 *  @caller client code, Agenda.purge(), Job.remove()
 */
Agenda.prototype.cancel = function(query, cb) {
  this._dbAdapter.cancel(query, cb);
};
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    this._dbAdapter.saveByID(id, props, processDbResult);
  } else if (props.type == 'single') {
    if (props.nextRunAt && props.nextRunAt <= now) {
      insertOnly.nextRunAt = props.nextRunAt;
      delete props.nextRunAt;
    }

    // Try an upsert.
    this._dbAdapter.saveSingle(props.name, 'single', props, insertOnly, processDbResult);
  } else if (unique) {
    ...
    this._dbAdapter.saveUnique(name, query, props, insertOnly, processDbResult);
  } else {
    this._dbAdapter.insert(props, processDbResult);
  }
  ...
  function processDbResult(err, result) {
    if (err) {
      ...
      job.attrs._id = result.id;
      job.attrs.nextRunAt = result.nextRunAt;
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  // Don't try and access Db if we've lost connection to it. Also see clibu_automation.js db.on.close code. NF 29/04/2015
  // Trying to resolve crash on Dev PC when it resumes from sleep.
  if (!this._dbAdapter.hasConnection()) {
    cb(new Error( 'No DB Connection'));
  } else {
    this._dbAdapter._findAndLockNextJob(jobName, this._nextScanAt, lockDeadline, function (err, result) {
      var job;
      if (!err && result) {
...
function processJobs(extraJob) {
  ...
  function lockOnTheFly() {
    ...
    self._dbAdapter.lockOnTheFly(job, function(err, resp) {
      if (resp) {
