...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  if ( this._mdb.s.topology.connections().length === 0 ) {
    ...
    this._collection.findOneAndUpdate(
      ...
      {returnOriginal: false, 'priority': -1},          // options
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    var now = new Date();
    self._collection.findOneAndUpdate({ _id: extraJob.attrs._id, lockedAt: null, disabled: { $ne: true } }, { $set: { lockedAt: now } }, { returnOriginal: false }, function(err, resp) {
