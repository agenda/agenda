...
Agenda.prototype.db_init = function( collection ){
    var self = this;
    this._mdb.collection( collection || 'agendaJobs', function( error, _collection ){
        self._collection = _collection;
        // Refactored to use createIndexes() and throw in `err` NF 10/05/2015
        self._collection.createIndexes( [ { key: { 'nextRunAt': 1 },
                                            name: 'nextRunAt',
                                          },
                                          {
                                            key: { 'lockedAt': 1 },
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    var now = new Date();
    self._collection.findAndModify({ _id: extraJob.attrs._id, lockedAt: null }, {}, { $set: { lockedAt: now } }, function(err, resp) {
