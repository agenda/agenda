...
Agenda.prototype.schedule = function(when, names, data, lastcb) {
  var self = this;

  if (typeof names === 'string' || names instanceof String) {
    return createJob(when, names, data, lastcb);
  } else if (Array.isArray(names)) {
    return createJobs(when, names, data, lastcb);
  }
  ...
  function createJobs(when, names, data, cb) {
    return names.map(function (name) {
      return createJob(when, name, data, cb);
    });
  }
};
