...
Agenda.prototype.database = function(url, collection, options) {
  ...
  var db=mongo(url,[collection]);
  var _collection= db.collection(collection);
  _collection.ensureIndex({"name" : 1, "priority" : -1, "lockedAt" : 1, "nextRunAt" : 1, "disabled" : 1}, ignoreErrors);
  _collection.ensureIndex({ "name" : 1, "lockedAt" : 1, "priority" : -1, "nextRunAt" : 1, "disabled" : 1 } , ignoreErrors);
...
Agenda.prototype.jobs = function(query,cb) {
  ...
  // }
  //console.log(this._db._collection.find(args));
  //return this._db._collection.find(args)
  return this._db._collection.find(query,findJobsResultWrapper(this,cb));
};

Agenda.prototype.purge = function(cb) {
  var definedNames = Object.keys(this._definitions);
  this._db._collection.remove({name: {$not: {$in: definedNames}}}, cb);
};
...
Agenda.prototype.cancel = function(query, cb) {
  return this._db._collection.remove(query, cb);
};
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    this._db._collection.findAndModify({query:{_id: id}, update:update, new: true}, processDbResult);
    ...
    // Try an upsert.
    this._db._collection.findAndModify({query:{name: props.name, type: 'single'}, update:update, upsert: true, new: true}, processDbResult);
  } else if (unique) {
    var query = job.attrs.unique;
    query.name = props.name;
    this._db._collection.findAndModify({query:query, update: update, upsert: true, new: true}, processDbResult);
  } else {
    this._db._collection.insert(props, processDbResult);
  }
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  this._db._collection.findAndModify(
    {
    query:{
      $or: [
...
function unlockJobs(done) {
  ...
  this._db._collection.update({_id: { $in: jobIds } }, { $set: { lockedAt: null } }, {multi: true}, done);
}
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    self._db._collection.findAndModify({
      query:{ _id: extraJob.attrs._id, lockedAt: null, disabled: { $ne: true } }, 
      update: { $set: { lockedAt: now } }
    }, 
      function(err, resp) {
      if (resp) {
