...
Agenda.prototype.create = function(name, data) {
  ...
  var job = new Job({name: name, data: data, type: e.attrs.type.normal, priority: priority, agenda: this});
  return job;
};
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if(id) {
    ...
    // Try an upsert.
    this._db.findAndModify({name: props.name, type: e.attrs.type.single}, {}, update, {upsert: true, new: true}, processDbResult);
  } else {
