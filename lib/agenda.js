...
Agenda.prototype.every = function(interval, names, data, options, cb) {
   ...
   */
  const createJob = function(interval, name, data, options, cb) {
   ...
   */
  const createJobs = function(interval, names, data, options, cb) {
    const results = [];
...
Agenda.prototype.schedule = function(when, names, data, cb) {
   ...
   * @returns {module.Job} instance of new job
   */
  const createJob = function(when, name, data, cb) {
    const job = self.create(name, data);
    job.schedule(when);
    job.save(cb);
    return job;
   ...
   */
  const createJobs = function(when, names, data, cb) {
    const results = [];
    let pending = names.length;
    let errored = false;
    return names.map((name, i) => {
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  if (s.topology.connections().length === 0) {
    ...
    */
    const JOB_RETURN_QUERY = {returnOriginal: false, priority: -1};
...
Agenda.prototype._unlockJobs = function(done) {
  const jobIds = this._lockedJobs.map(job => {
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    for (const jobName in definitions) {
  ...
  function lockOnTheFly() {
    ...
    self._collection.findOneAndUpdate(criteria, update, options, (err, resp) => {
      // NOTE: This error should be handled instead of just being rethrown
      if (err) {
        throw err;
      }
