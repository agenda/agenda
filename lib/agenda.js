 ...
 *********************************/
 ...
 Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    db.findAndModify(self.client, self.tableName, false, update, [{_id: id}], false, "joinCondition",true, processDbResult);
...
Agenda.prototype.cancel = function(query, cb) {
  var sqlQuery = db.getDeleteQuery(this.tableName, condition, 'AND');
 ...
 *********************************/
 ...
 function processJobs(extraJob) {
   ...
   if (!extraJob) {
     ...
     var now = new Date();
     db.findAndModify(self.client, self.tableName, false, [{locked_at: now.valueOf()}], [{ _id: extraJob.attrs._id, locked_at: null }], false, "AND", false, function(err, resp){
       if (resp) {
 ...
 Agenda.prototype._findAndLockNextJob = function(jobName, definition, callback) {
   ...
   var sqlQuery = "SELECT * FROM " + self.tableName + " WHERE name = '" + jobName + "' AND (locked_at is null OR locked_at <= '" + lockDeadline.valueOf()   + "') AND next_run_at <= '" + self._next_scan_at.valueOf()  + "' ORDER BY priority DESC LIMIT 1";
   db.executeSql(self.client, sqlQuery, function(err, result){
     if(err){
       ...
       if(result.length){
         var updateQuery = "UPDATE " + self.tableName + " SET locked_at = '" + now.valueOf()  + "' WHERE name = '" + jobName +"'";
 ...
 function unlockJobs(done) {
   ...
   var sqlQuery = db.getUpdateQuery(this.tableName, [{locked_at: null}], [{_id: { $in: jobIds } }], "AND");
   db.executeSql(this.client, sqlQuery, function(err, result){
     if(err){
