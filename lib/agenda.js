...
Agenda.prototype.jobs = function () {
  ...
  if (typeof args[args.length - 1] == 'function') {
    args.push(_findJobsResultWrapper(this, args.pop()));
  }
...
Agenda.prototype.define = function (name, options, processor) {
  ...
  this._definitions[name] = {
    ...
    lockLifetime: options.lockLifetime || 5 * 60 * 1000,//5 minute default lockLifetime
    running: 0
  };
};
...
Agenda.prototype._findAndLockNextJob = function (jobName, definition, cb) {
  var now = new Date,
      lockDeadline = new Date(Date.now() - definition.lockLifetime);
  ...
  this._db.findAndModify(
    ...
    {$set: {lockedAt: now}},
    {'new': true},
    _findJobsResultWrapper(this, cb)
  );
};
 ...
 * @private
 */
function _findJobsResultWrapper(agenda, cb) {
  return function (err, jobs) {
    if (jobs) {
      //query result can be array or one record
      if (jobs instanceof Array) {
        jobs = jobs.map(_createJob.bind(null, agenda))
      } else {
        jobs = _createJob(agenda, jobs);
      }
    }
 ...
 * @private
 */
function _createJob(agenda, jobData) {
