...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  if (this._mdb.s.topology.connections().length === 0) {
    ...
    this._collection.find({
      $or: [
        {name: { $regex: jobName }, lockedAt: null, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }},
        {name: { $regex: jobName }, lockedAt: {$exists: false}, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }},
        {name: { $regex: jobName }, lockedAt: {$lte: lockDeadline}, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }}
      ]
    })
    .limit(Math.min(self._lockLimit - self._lockedJobs.length, definition.lockLimit <= definition.locked))
    .toArray(function (err, result) {
      ...
      if (!err && result.length) {
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          jobs.push(createJob(self, definition, result[i]));
        }
      }
      cb(err, jobs);
    });
 ...
 * @private
 */
function createJob(agenda, definition, jobData) {
  if (jobData === undefined) {
    jobData = definition;
    var definition = agenda._definitions[jobData.name];
  }

  jobData.agenda = agenda;
  jobData.definition = definition;
...
function processJobs(extraJob) {
  ...
  function processJobResult(err, job) {
    ...
    job.definition.running--;

    self._lockedJobs.splice(self._lockedJobs.indexOf(job), 1);
    job.definition.locked--;
