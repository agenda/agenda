...
class Agenda extends Emitter {
  constructor(config, cb) {
    ...
    this._isReady = false;
    const self = this;
    function callback(err, collection) { // Overrides cb to set _isReady parameter
      self._isReady = !err;
      if (!cb && err) {
        throw err;
      }
    ...
    if (config.mongo) {
      this.mongo(config.mongo, config.db ? config.db.collection : undefined, callback);
    } else if (config.db) {
      this.database(config.db.address, config.db.collection, config.db.options, callback);
    }
  }
}
...
Agenda.prototype.isReady = require('./is-ready');
