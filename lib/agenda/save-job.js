...
module.exports = async function(job) {
  try {
    ...
    if (unique) {
      ...
      const result = await this._collection.findOneAndUpdate(query, update, {...job.mongoOptions, upsert: true, returnOriginal: false});
      return processDbResult(job, result);
    }
    ...
    const result = await this._collection.insertOne(props, job.mongoOptions);
    return processDbResult(job, result);
  } catch (err) {
