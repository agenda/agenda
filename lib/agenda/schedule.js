 ...
 * @param {Object} mongoOptions MongoDB options to use at job creation
 * @returns {Promise<Job|Job[]>} job or jobs created
 */
module.exports = function(when, names, data, {session} = {}) {
  const self = this;
   ...
   */
  const createJob = async(when, name, data, {session} = {}) => {
    const job = self.create(name, data, {session});
   ...
   */
  const createJobs = async(when, names, data, {session} = {}) => {
    try {
      const jobs = await Promise.all(names.map(name => createJob(when, name, data, {session})));
