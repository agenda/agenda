  ...
  client.query(sqlStatement, function(err, results){
    ...
    if (err) {
      ...
      if(results.rows.length){
        if(typeof parseInt(results.rows[0].next_run_at) === 'number'){
          results.rows[0].next_run_at = new Date(parseInt(results.rows[0].next_run_at));
        }
        if(typeof parseInt(results.rows[0].last_run_at) === 'number'){
          results.rows[0].last_run_at = new Date(parseInt(results.rows[0].last_run_at));
        }
        if(typeof parseInt(results.rows[0].failed_at) === 'number'){
          results.rows[0].failed_at = new Date(parseInt(results.rows[0].failed_at));

        }
        if(typeof parseInt(results.rows[0].last_finished_at) === 'number'){
          results.rows[0].last_finished_at = new Date(parseInt(results.rows[0].last_finished_at));
        }
        if(typeof parseInt(results.rows[0].locked_at) === 'number'){
          results.rows[0].locked_at = new Date(parseInt(results.rows[0].locked_at));
        }
  ...
  for(var i = 0 ; i <keys.length ; ++i){
    if(typeof values[0][keys[i]] == 'object'){
      ...
      if(keys[i] == 'data'){
        ...
        var date = new Date(x);
        sqlQuery = sqlQuery  + "\'" + date.valueOf() + "\'";
...
var getDeleteQuery = function(tableName, condition, conditionArray, joinCondition){
  ...
  if(condition){
    if(condition != '{}'){
      var keys = getKeys(condition);
      sqlQuery = sqlQuery + " WHERE ";
      sqlQuery = processQuery(sqlQuery, condition, joinCondition);
    }
  }
  return sqlQuery;
};
      ...
      if(typeof condition[0][keys[i]] == 'object'){
       ...
       if(keys[i] === 'data'){
        ...
        if(i < (keys.length - 1)){
         ...
         var date = new Date(value);
         sqlQuery = sqlQuery + keys[i] + " = \'" + date.valueOf() + "\'";
         if(i < (keys.length - 1)){
           sqlQuery = sqlQuery + " " + joinCondition + " ";
         }
      }
         ...
         for(var key in value) {
           ...
           if(val instanceof Date){
             ...
             sqlQuery = sqlQuery + keys[i] + operator + "'" + date.valueOf() + "'";
           }
           else{
