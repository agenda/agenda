...
Job.prototype.computeNextRunAt = function() {
    ...
    function computeFromInterval() {
        ...
        var lastRun = this.attrs.lastRunAt || dateNow;
        var startDate = this.attrs.startDate;
        var endDate = this.attrs.endDate;
        var skipDays = this.attrs.skipDays;
        ...
        try {
            ...
            if (startDate != null) {
                startDate = moment.tz(moment(startDate).format('YYYY-MM-DD'), timezone);
                if (startDate > nextDate) {
                    nextDate = cronTime._getNextDateFrom(dateForTimezone(startDate));
                }
            }
            ...
            if ((dateNow > lastRun) && skipDays != null) {
                try {
                    nextDate = nextDate.valueOf() + humanInterval(skipDays);
                } catch (e) {}
            }

            //If endDate is less than the nextDate, set nextDate to null to stop the job from running further
            if (endDate != null) {
                endDate = moment.tz(moment(endDate).format('YYYY-MM-DD'), timezone);
                if (nextDate > endDate) {
                    nextDate = null;
                }
            }
...
Job.prototype.repeatEvery = function(interval, options) {
    ...
    this.attrs.startDate = options.startDate ? options.startDate : null;
    this.attrs.endDate = options.endDate ? options.endDate : null;
    this.attrs.skipDays = options.skipDays ? options.skipDays : null;
    return this;
};
