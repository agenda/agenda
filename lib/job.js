...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromInterval() {
    ...
    try {
      ...
      _nextRun = new Date(this.attrs.nextRunAt);
      _startsRun = new Date(this.attrs.startRunsAt);
      _endsRun = new Date(this.attrs.noMoreAt);

      if (_nextRun < _startsRun) {
          this.attrs.nextRunAt = _startsRun;

      } else if (_nextRun > _endsRun) {
