...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromRepeatAt() {
    var lastRun = this.attrs.lastRunAt || new Date();
    lastRun = dateForTimezone(lastRun);
    var nextDate = dateForTimezone(date(repeatAt)).valueOf();
    ...
    if (offset === date(repeatAt,offset).valueOf()) {
      ...
      this.attrs.nextRunAt = dateForTimezone(date(repeatAt)) + humanInterval(interval);
    } else {
      this.attrs.nextRunAt = dateForTimezone(date(repeatAt));
    }
  }
};
...
Job.prototype.schedule = function(time, options) {
  ...
  if (timezone) {
    var d = moment(this.attrs.nextRunAt);
    this.attrs.nextRunAt = d.tz(timezone);
  }
