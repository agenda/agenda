...
module.exports = function() {
  ...
  /**
   * Internal method to compute next run time using RRule
   * @returns {undefined}
   */
  function computeFromRrule() {
    debug('[%s:%s] computing next run via rrule [%o]', this.attrs.name, this.attrs._id, rrule);
    const rruleObject = new RRule(rrule);
    const lastRunAt = this.attrs.lastRunAt || new Date();
    const nextDate = rruleObject.after(lastRunAt);
    if (!nextDate) {
      debug('[%s:%s] no more dates to run. nextRunAt set to undefined', this.attrs.name, this.attrs._id);
      this.attrs.nextRunAt = undefined;
    }
    else {
      this.attrs.nextRunAt = nextDate;
