...
module.exports = function() {
  ...
  const computeFromInterval = () => {
    ...
    try {
      ...
      // If startDate exists
      if (startDate !== null) {
        // If startDate is in future, nextDate is moved after startDate
        if (startDate > nextDate) {
          nextDate = cronTime._getNextDateFrom(startDate);
        } else if (!this.attrs.lastRunAt) {
          // This is the first run and startDate is in past, find nextDate in future
          nextDate = cronTime._getNextDateFrom(startDate);
          let dateOffset = cronTime._getNextDateFrom(startDate);
          while (dateOffset <= nextDate) {
            dateOffset = cronTime._getNextDateFrom(dateOffset.valueOf());
          }
          nextDate = dateOffset;
        }
      }
      ...
      try {
        ...
        // Check if startDate is available
        if (startDate !== null) {
          // If startDate is in future, nextDate is moved after startDate
          if (startDate.valueOf() > this.attrs.nextRunAt) {
            this.attrs.nextRunAt = startDate.valueOf();
          } else if (!this.attrs.lastRunAt) {
            // This is the first run and startDate is in past, find nextDate in future
            let dateOffset = startDate.valueOf();
            while (dateOffset <= this.attrs.nextRunAt) {
              dateOffset += _convertedInterval;
            }
            this.attrs.nextRunAt = dateOffset;
          }
        }
