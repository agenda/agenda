...
module.exports = function() {
  ...
  return new Promise(async(resolve, reject) => {
    ...
    try {
      ...
      if (!definition.fn) {
        debug('[%s:%s] has no definition function, can not run', self.attrs.name, self.attrs._id);
        throw new Error('Undefined job function');
      }
      if (definition.fn.length === 2) {
        debug('[%s:%s] process function being called with callback', self.attrs.name, self.attrs._id);
