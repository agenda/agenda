...
module.exports = function(extraJob) {
  ...
  function processJobResult(err, job) {
    // There are four possiblities for this callback:
    //
    //  1. done() was called with no error and saving succeeded
    //  2. done() was called with no error and saving failed
    //  3. done(err) was called and saving succeeded
    //  4. done(err) was called and saving failed
    //
    // The err param can either represent the error passed to done() or the one
    // returned when saving fails (in that order). However, there is no job
    // provided when saving fails. This check catches cases 2 and 4 above (save
    // failure). After this point we know that the job was successfully
    // persisted and any value in err represents the error passed to done().
    if (err) {
