...
module.exports = function(extraJob) {
  ...
  function jobProcessing() {
    ...
    /**
      * Internal method that checks if all concurrency requirements are fulfilled
      * @returns {boolean}
    */
    function fulfillsConcurrencyLimits() {
      if (self._runningJobs.length < self._maxConcurrency && jobDefinition.running < jobDefinition.concurrency) {
        return true;
      }
      if (self._maxConcurrency < 0 && jobDefinition._maxConcurrency < 0) {
        return true;
      }
      if (self._maxConcurrency < 0 && jobDefinition.running < jobDefinition.concurrency) {
        return true;
      }
      if (jobDefinition.concurrency < 0 && self._runningJobs.length < self._maxConcurrency) {
        return true;
      }
