...
describe('Job', () => {
	...
	describe('computeNextRunAt', () => {
		...
		describe('when repeat interval is invalid', () => {
			...
			it('fails the job', () => {
				expect(job.attrs.failReason).to.match(
					/failed to calculate nextRunAt due to invalid repeat interval \(asd\): Error:/
				);
			});
		});
	});
	...
	describe('Integration Tests', () => {
		describe('.every()', () => {
			it('Should not rerun completed jobs after restart', async () => {
				let i = 0;
					...
					return new Promise<void>((resolve, reject) => {
						...
						n.on('message', (msg) => {
							if (msg === 'ran') {
								...
								startService().then(resolve).catch(reject);
							} else if (msg === 'notRan') {
								expect(i).to.equal(1);
								resolve();
							} else {
			...
			it('Should properly run jobs when defined via an array', async () => {
				const serverPath = path.join(__dirname, 'fixtures', 'agenda-instance.ts');
				...
				await new Promise<void>((resolve, reject) => {
					...
					n.on('error', reject);
					setTimeout(() => reject(new Error('Jobs did not run!')), 5000);
				});
			});
		...
		describe('schedule()', () => {
			it('Should not run jobs scheduled in the future', async () => {
				await new Promise<void>((resolve, reject) => {
					const serverPath = path.join(__dirname, 'fixtures', 'agenda-instance.ts');
			...
			it('Should run past due jobs when process starts', async () => {
				await new Promise<void>((resolve, reject) => {
					const serverPath = path.join(__dirname, 'fixtures', 'agenda-instance.ts');
			...
			it('Should schedule using array of names', async () => {
				const serverPath = path.join(__dirname, 'fixtures', 'agenda-instance.ts');
		...
		describe('now()', () => {
			it('Should immediately run the job', async () => {
				const serverPath = path.join(__dirname, 'fixtures', 'agenda-instance.ts');
				const n = cp.fork(serverPath, [mongoCfg, 'now'], { execArgv: ['--import', 'tsx'] });
				...
				await new Promise<void>((resolve, reject) => {
					n.on('message', (msg) => {
						if (msg === 'ran') {
							...
							reject(new Error('Job did not immediately run!'));
						}
					});
					n.on('error', reject);
				});
