    ...
    describe('Job', function() {
        ...
        describe('computeNextRunAt', function() {
            ...
            it('understands delayed start when start date (10 days from now) with cron interval and timezone', function() {
                ...
                job.computeNextRunAt();
                expect(moment(job.attrs.nextRunAt).tz('GMT').hour()).to.be(6);
                expect(moment(job.attrs.nextRunAt).toDate().getDate()).to.be(moment(startDate).toDate().getDate());
            });
        ...
        describe('job concurrency', function() {
            ...
            it('should not block a job for concurrency of another job', function(done) {
                ...
                jobs.schedule(new Date(now + 100), 'blocking', {
                    i: 1
                });
                ...
                setTimeout(function() {
                    ...
                    jobs.schedule(new Date(now + 100), 'non-blocking', {
                        i: 3
                    });
                }, 100);
            });
